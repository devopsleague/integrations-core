...
def manifest(ctx: click.Context, integrations: tuple[str, ...]):
    ...
    if integrations:
        selected_integrations = [app.repo.integrations.get(integration) for integration in integrations]
    else:
        selected_integrations = list(app.repo.integrations.iter_changed())
        ...
        try:
            ...
            if response.status_code == 400:
                for error in response.json()['errors']:
                    validation_tracker.error((integration.name, 'manifest.json'), message=error)
            else:
                response.raise_for_status()
                validation_tracker.success()
        except Exception as e:
            validation_tracker.error((integration.name, 'manifest.json'), message=str(e))
